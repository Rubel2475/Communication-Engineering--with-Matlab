...
for i=0:total_bits-1;
    if data(i+1) == 0
      if previous == 1;
        ...
        y((2*i+1)*(sample_per_bit/2)+1 : (i+1)*sample_per_bit) = 1;
        previous = 1;
      else
        y(i*sample_per_bit+1 : (2*i+1)*(sample_per_bit/2)) = 1;
        y((2*i+1)*(sample_per_bit/2)+1 : (i+1)*sample_per_bit) = -1;
      ...
      y(i*sample_per_bit+1 : (2*i+1)*(sample_per_bit/2)) = previous;
      y((2*i+1)*(sample_per_bit/2)+1 : (i+1)*sample_per_bit) = -previous;
      previous = -previous;
    endif;
endfor;
